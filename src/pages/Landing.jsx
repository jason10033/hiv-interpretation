...
const ROUTES = [
  {
    id: 'lookup',
    icon: '✓',
    label: 'I just want to know what to do',
    sublabel: 'Enter your results, get an interpretation',
    path: '/lookup',
    color: '#162447',
    bg: 'var(--primary-bg)',
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '32px 16px 48px' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.2, marginBottom: 8 }}>
        ...
        {ROUTES.map((route) => (
          ...
          >
            <span style={{
              ...
              background: 'white', borderRadius: 10, boxShadow: 'var(--shadow)',
              flexShrink: 0, color: route.color,
            }}>{route.icon}</span>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: route.color, lineHeight: 1.3, marginBottom: 2 }}>
                {route.label}
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{route.sublabel}</div>
            </div>
            <span style={{ marginLeft: 'auto', color: route.color, opacity: 0.5, fontSize: 18, flexShrink: 0 }}>›</span>
          </button>
        ))}
      </div>
      <ClinicalDisclaimer />
